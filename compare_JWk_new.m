...
Hk0_store = cell(L/2-1,1);
phik0_1 = zeros(L/2-1,1);
phik0_2 = zeros(L/2-1,1);
e_GS = zeros(L/2-1,1);

%% initial state
for i = 1:L/2-1
    Hk0 = 2*[g0+J0*cospi(k_r(i)) -1i*J0*sinpi(k_r(i));
                1i*J0*sinpi(k_r(i)) -g0-J0*cospi(k_r(i))];
    Hk0_store{i} = Hk0;
    [V,D] = eig(Hk0);
    phik0_1(i) = V(1,1);
    phik0_2(i) = V(2,1);
...
e_GS_sum = sum(e_GS)-2*abs(J0-g0)-2*abs(J0+g0) - 4*g0;
...
expHkxx_11 = cc + 1i*ss*cospi(k_r);
expHkxx_22 = conj(expHkxx_11);
expHkxx_12 = -ss*sinpi(k_r);
...
Cr = 2*(abs(phik_1).^2)'*cospi(k*xx')/L;
