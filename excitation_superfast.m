...
expHx0 = sparse(1:len,len:-1:1,(-1i)^L);

expHx1 = sigmax;
expHx2 = sparse(1:4,1:4,1);
expHx3 = sparse(1:8,1:8,1);
% expHx4 = sparse(1:16,1:16,1);
expHx1 = kron(sI2, expHx1) + kron(sigmax, sparse(1:2,1:2,1));

expHx2 = kron(sI2, expHx2) + kron(sigmax, expHx1);
expHx1 = kron(sI2, expHx1) + kron(sigmax, sparse(1:4,1:4,1)); 

% expHx3 = kron(sI2, expHx3) + kron(sigmax, expHx2);
% expHx2 = kron(sI2, expHx2) + kron(sigmax, expHx1);
% expHx1 = kron(sI2, expHx1) + kron(sigmax, sparse(1:8,1:8,1)); 

for i = 4:L
%     expHx4 = kron(sigmax, expHx4) + kron(I2, expHx3);
    expHx3 = kron(sI2, expHx3) + kron(sigmax, expHx2);
    expHx2 = kron(sI2, expHx2) + kron(sigmax, expHx1);
    expHx1 = kron(sI2, expHx1) + kron(sigmax, sparse(1:2^(i-1),1:2^(i-1),1)); 
end
...
expHx1 = sparse(row1,col1,cos(epsilon)^(L-1)*-1i*sin(epsilon));
expHx2 = sparse(row2,col2,cos(epsilon)^(L-2)*(-1i*sin(epsilon))^2);
expHx3 = sparse(row3,col3,cos(epsilon)^(L-3)*(-1i*sin(epsilon))^3);
% expHx4 = sparse(row4,col4,cos(epsilon)^(L-4)*(-1i*sin(epsilon))^4);
clear row1 row2 row3 col1 col2 col3
expHxL = sparse(1:len,1:len,cos(epsilon)^L);
